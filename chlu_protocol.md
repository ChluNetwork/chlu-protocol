...
implemented by wallets. Marketplaces that want to integrate with Chlu
do so through libraries.
...
inside marketplace walled gardens as well as reputation data being
potentially faked by vendors.
...
Each of these componets can be provided by any one, as long as these
components follow the protocol described here. To be clear the terms
'vendor wallet provider', 'customer wallet provider' and 'market place
library provider' are used in this document to mean any open source
project or company providing an implementation of the appropriate
component.
...
enclosing one or more ```ReviewAttribute```s. If included in the PoPR,
...
be_present | Is this attribute required to be completed
...
2. Sign a PoPR when directly sending a payment request.
...
1. Enough information so that key pairs for all the blockchains used
   ...
   be exported as mnemonic and key paths, but others like Ethereum
...
statistics and algorithm to derive vendor reputation scores, but they
should not make any validatable review inaccessible to its users.
