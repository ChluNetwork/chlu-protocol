...
validated by payments made via permissionless cryptocurrencies like
...
advantage of such systems by creating fake reviews for
...
Each of these components can be provided by any third party, as long as
...
key_location | The IPFS content address where the marketplace specific vendor key is available. This key is signed by vendor's key available at `vendor_key_location`.
...
vendor_key_location | The IPFS content address where the vendor's public key is available. This key is used to sign the key available at `key_location`.
vendor_encryption_key_location | The IPFS content address where the vendor's public key for encrypting reviews is available. This key is signed by vendor's key available at `vendor_key_location`.
...
customer using Facebook App Links or Firebase Dynamic Links for the
wallet provider to send the PoPR.
...
prev | The IPFS location of the most recent review record created by the customer, if any.
...
from the hash) are encrypted using the vendor public encryption key
accessible from IPFS. The location of the encryption key is included
in the PoPR.
...
key pair should be generated to be used for signing the PoPR. The key
pair should be an elliptical curve key pair using the same curve as
...
The public key, Pv, is sent to the marketplace when the vendor
registers with a marketplace. Any reviews that have been signed by
keys that are in turn signed by Pv are considered to be reviews for
sales made by the vendor V.
...
exported key to reinitialised the wallet.

### Setup Encryption key pair (Sve, Pve) ###

Wallets have to generate another key pair of type RSA with 2048
length. The public key of this key pair is signed by the key Sv and
the signed public key is then saved on IPFS. The location of this
signed encryption key Pve is sent to the marketplace during vendor
registration.
...
and publishing them on IPFS](./images/vendor-wallet-init.png)
...
new keys for all the cryptocurrencies supported by the wallet.
...
### Setup Review Signing Keys ###

Customer wallets should generate a key pair for signing reviews. The
key pair should be an elliptical curve key pair using the same curve
as the one used in bitcoin - secp256k1.

This key is used by the customer wallet to update the review or even
...
It is up to the wallet provider how the customer signing key is
created and safe guarded. The wallet also has to make sure this key is
exported when the wallet is exported, as described in the Exporting
Wallet section.
...
in turn holds a reference to the Proof of Payment Request.
...
## Time stamping Review ##

Whenever a customer creates or updates a review, the customer wallet
should include a hash to the most recent review record created by the
customer. The hash is saved in the ```prev``` attribute of the review
record. This attribute can be blank if this is first ever review
created by a customer. This chain of hashes between a customer
authored review allow marketplace to get the time elapsed between a
review creation and review update. This limits the potential of a
customer to sell review update rights to an attacking vendor.

## Updating a review

All the reviews by a customer are signed by the customer signing key
created during customer wallet setup. A customer can create a new
review without requiring any permission. 

Each new review has to include the PoPR that resulted in the
payment. Without a valid PoPR, a review is considered invalid, and
should not be used to compute a vendor's reputation score. Review
updates instead do not include a PoPR but have a hash pointer to the
previous review that is being updated.

The above proves that the the customer is overwriting the earlier
...
2. A vendor runs a service to push reviews to a marketplace. The
   ...
   the payment captured by customer_address, vendor_address and the
   currency symbol
...
2. Fetch the vendor public signature key, Pvm from IPFS location
   included in the PoPR's ```key_location``` attribute.
2. Fetch the vendor public signature key, Pv from IPFS location
   included in the PoPR's ```vendor_key_location``` attribute.
2. Fetch the vendor public encryption key, Pve from IPFS location
   included in the PoPR's ```vendor_encryption_key_location``` attribute.
3. Verify that Pv is the same as that sent to the marketplace by the
   vendor at registration time.
4. Validate that Pve is signed by Pv.
5. Validate that PoPR is signed by Pvm.
6. Validate that Pvm is signed by Pv and Pm.
4. Verify that the contents of the Review Record result in the same
   hash as included in the Review Record.

If any of the verification in the above procedure fail, the
...
Any third party, including vendor wallets can verify that marketplaces
are listing all the reviews and doing so correctly. This prevents
marketplaces from selecting hiding or showing incorrect reviews for a
vendor.

# Versioning

The protocol buffers serialisation format is designed to be
